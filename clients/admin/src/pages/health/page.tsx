  ...
  return (
    <div className="space-y-8">
      <EntityPageHeader
        ...
        description={
          <>
            Живые пробы процесса и зависимостей. Автообновление каждые {REFRESH_INTERVAL_MS / 1000} с.
            Сами эндпоинты проб (<code className="code-chip">/health/live</code>{" "}
            <code className="code-chip">/health/ready</code>) без авторизации — их читают
            балансировщики и мониторинги аптайма.
          </>
        }
      >
      ...
      {live.isError && (
        <ErrorBand
          kind="liveness"
          message={live.error instanceof Error ? live.error.message : "Проба liveness не прошла."}
        />
      )}
      {ready.isError && (
        <ErrorBand
          kind="readiness"
          message={ready.error instanceof Error ? ready.error.message : "Проба readiness не прошла."}
        />
      )}
      ...
      <ProbeSection
        ...
        description="Процесс поднят и обслуживает запросы. Внешние зависимости не проверяются."
      />
      ...
      <ProbeSection
        ...
        description="Все проверки зависимостей должны быть Healthy, чтобы проба вернула 200. 503 здесь выводит API из пула балансировщика."
      />
    </div>
  );
}
...
function ProbeSection({
  ...
  return (
    ...
    >
      {loading ? (
        <div className="py-6 text-sm text-[var(--color-muted-foreground)]">Опрос…</div>
      ) : !result || result.results.length === 0 ? (
        <div className="flex items-center gap-3 py-5">
          <Activity className="h-4 w-4 text-[var(--color-muted-foreground)]" />
          <div>
            <div className="text-sm font-medium">Проверки зависимостей не пришли.</div>
            <div className="text-xs text-[var(--color-muted-foreground)]">
              Проба ответила со статусом <code className="code-chip">{result?.status ?? "—"}</code>.
            </div>
          </div>
        </div>
      ) : (

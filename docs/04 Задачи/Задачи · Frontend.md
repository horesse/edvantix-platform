...
- [x] «Tenant» → «Школа» во всём UI: списки, формы, диалоги, тексты ошибок,
      навигация, командная палитра, тексты подтверждений. API-поля/маршруты не
      тронуты. Заодно приложение полностью локализовано на русский и
      перебрендировано под Edvantix (`brand-mark`, страницы входа)
- [x] Карточка школы: блок показателей — активные ученики/преподаватели/группы/
      занятий за месяц/объём файлов из снимков `GET /api/v1/billing/usage?tenantId=`
      (`SchoolMetricsCard`, гейт `Billing.View`). Готового live-агрегата нет —
      показан последний захваченный период; если снимков ещё нет — явная подсказка
- [x] Планы: лимиты и ставки за превышение в терминах учеников/преподавателей/
      групп/занятий (`RESOURCE_LABEL`, `OVERAGE_RESOURCES`); добавлено поле
      «Описание» плана (форма + карточка), доменные `QuotaResource` в `api/billing.ts`
- [x] Дашборд платформы: «Обзор платформы» — KPI школ (всего/активные/истекают
      ≤45 дн/у лимитов ≥80%), «Школы по тарифам» (fan-out `GET /tenants/{id}/status`
      при ≤60 школ), «Истекающие подписки», «Приближаются к лимитам» (снимки
      `GET /billing/usage`). «Рост» опущен — у школ нет `createdUtc`
- [x] Каталог событий вебхуков — `GET /api/v1/webhooks/event-types`
      (`listWebhookEventCatalog`), чек-лист с группировкой по модулям в диалоге
      создания подписки (`webhook-labels.ts`), свободный ввод для forward-compat,
      тумблер «все события `*`»; офлайн-fallback синхронен `WebhookEventCatalog.All`
- [x] Демо-аккаунты (`pages/login.demo-accounts.ts`,
      `components/auth/demo-accounts-dialog.tsx`): операторские аккаунты корневой
      школы (`admin@root.com` + `superadmin@root.com`), персоны под Edvantix,
      комментарий «keep in sync» переведён на `DemoSeeder.BuildRootUsers()`.
      Роли школы (менеджер/преподаватель/представитель/ученик) сюда не добавлены:
      это операторская консоль, эти пользователи живут в dashboard, а маршруты
      admin гейтятся root-правами (изоляция тенантов — врезка ниже)
- [x] Брендирование `components/brand-mark.tsx` + лого на страницах входа — Edvantix

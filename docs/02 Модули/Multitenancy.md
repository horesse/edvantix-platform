...
| `TenantSettings` | часовой пояс (IANA `TimeZoneId`, по умолчанию `UTC`), валюта (ISO 4217 `Currency`, по умолчанию `USD`), автоблокировка материалов при задолженности (`RestrictMaterialsOnDebt`, по умолчанию `false`; грейс `DebtGraceDays`, по умолчанию 7 — см. [[Payments]] → «Автоблокировка доступа к материалам»), шаблон номера счёта (`InvoiceNumberTemplate`, по умолчанию `{YYYY}-{NNNN}` — рендерит [[Payments]], см. «Нумерация счетов»; `null` в `UpdateTenantSettingsCommand` = «не менять») школы |
...
в `Edvantix.DbMigrator`. См. [[Бэклог]].

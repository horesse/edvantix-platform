...
GET    /api/v1/people/me/scope                 маршрут с сегментом /people/
GET    /api/v1/people/duplicate-candidates     мягкая проверка на дубль (EDX-018): ФИО +
                                               телефон/e-mail → Student/Teacher/Guardian;
                                               без прав, ничего не блокирует
```

Мягкий дедуп (EDX-018): точное совпадение фамилии + имени (`ILIKE` без шаблонов) **и**
совпадение телефона (сравнение по цифрам) **или** e-mail. Совпадения только по имени
отбрасываются — семья с одним контактным телефоном на нескольких детей не должна
поднимать предупреждение (та же причина, по которой нет уникального индекса по телефону).

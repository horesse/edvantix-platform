...
Настраивается на уровне тенанта (EDX-013). `TenantSettings.InvoiceNumberTemplate`
(в [[Multitenancy]], рядом с TZ/валютой), дефолт `{YYYY}-{NNNN}` — бэкофиллен для
существующих тенантов миграцией `AddInvoiceNumberTemplate`.

**Плейсхолдеры** (продуктовое решение, было в [[Открытые вопросы]]):

| Токен | Значение |
|---|---|
| `{YYYY}` / `{YY}` | год (4 / 2 цифры) |
| `{MM}` | месяц, 2 цифры |
| `{N}` … `{NNNNNNNNNN}` | счётчик, дополняется нулями до числа `N` в маске; значение шире маски печатается целиком |

Любой другой текст выводится как есть. Рендерит `InvoiceNumberFormat` (в `Modules.Payments`,
чистая функция без состояния); шаблон валидируется на записи в `UpdateTenantSettingsCommandValidator`.

**Политика обнуления выводится из шаблона**, отдельного тумблера нет: шаблон содержит
`{YYYY}`/`{YY}` → счётчик сбрасывается по календарным годам (scope = год); иначе сквозной
(scope = `*`). Смена шаблона посреди года **не** сбрасывает счётчик текущего scope; переход
со сквозного на по-годам заводит счётчик года с 1 (осознанное действие администратора).

**Счётчик — `payments."InvoiceNumberSequences"`** (`TenantId`, `Scope`, `NextValue`), одна
строка на scope. `IInvoiceNumberGenerator` резервирует непрерывный блок номеров одним
атомарным `INSERT … ON CONFLICT ("TenantId","Scope") DO UPDATE SET "NextValue" = "NextValue" +
EXCLUDED."NextValue" RETURNING "NextValue"` — `DO UPDATE` берёт блокировку строки, поэтому
конкурентные пакеты (класс целиком) сериализуются и их диапазоны не пересекаются. `NextAsync`
для одиночного счёта, `NextBatchAsync(n)` для `BulkGenerateInvoicesCommand`. Номер
присваивается при **создании** счёта (в т.ч. `Draft`); `StudentInvoice.Create` принимает
готовую строку, доменная фабрика номер больше не генерирует. Дата для `{YYYY}`/`{MM}` — UTC
(как и прежняя схема). Разрывы при откате транзакции допустимы и нормальны для нумерации.

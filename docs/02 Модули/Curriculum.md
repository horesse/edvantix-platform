...
> агрегата, мягкое удаление только у `Course`, семантика дублирования и т.д.) —
> `.agents/rules/modules/curriculum.md` и разделы «Проектные решения» ниже.
...
> и в [[Multitenancy]] → «Шаги провижининга под новые модули».
...
- Ровно одно из `FileId` / `Url` заполнено — валидатор + CHECK-ограничение. Какое
  именно — задаёт вид: `Video`/`Link` несут внешний `Url`, `File`/`Presentation` —
  `FileId`, `Homework` — любое. Проверяется доменом и `AddLessonMaterialCommandValidator`.
- `MaterialKind = Video` — только ссылка на разрешённый хостинг
  (`CurriculumOptions.VideoMaterialAllowedHosts`, по умолчанию YouTube, Vimeo, RuTube,
  VK, Дзен; совпадение по хосту URL и любому его поддомену). Прямая загрузка видео-файла
  невозможна: `Video` не принимает `FileId`, а видео-контейнеры исключены из белого
...
Прогресс группы по программе («прошли 12 из 40 уроков»). Не хранимая проекция:
`total` — уроки курса (уже здесь), `passed` — уникальные `LessonId` у проведённых
занятий [[Scheduling]]. Считается на лету запросом `GetGroupCourseProgressQuery`
(живёт в Scheduling, `GET /study-groups/{id}/course-progress`, EDX-019). Завести
проекцию (таблицу `curriculum` или `scheduling` с парой `(StudyGroupId → passedCount)`,
обновляемую подпиской на `SessionHeldIntegrationEvent`) стоит, когда карточка группы /
вкладка «Группы» ученика начнут ощутимо тормозить — ориентир: сотни занятий на группу
или запрос прогресса в горячем списке (сейчас — точечно, на карточку). До этого лишний
стор только добавляет рассинхрон.
...
[[ADR-002 Catalog заменяется на Curriculum]] · [[ADR-006 Урок программы и занятие расписания]] · [[Бэклог]]

...
регистрируют свои права. Подробности решений — [[Бэклог]].
...
реализован, замыкая последний пункт [[Бэклог]] по Identity. Гейт —
...
2. Роль ограничена `SchoolRoleConstants.All` — валидатор команды отбрасывает произвольную
   строку (сознательное решение первой итерации, см. [[Бэклог]]).
...
5. Письмо — тем же каналом, что `ForgotPasswordCommand` (`IMailService` + `IJobService.Enqueue`,
   ...
   собираются обычной C#-интерполяцией строки, как и раньше (то же ограничение
   отмечено в [[Notifications]] → «Шаблоны сообщений»). Ссылка —
...
раздел «Приглашение по e-mail» выше). В бандле роли

...
```bash
dotnet run --project src/Host/Edvantix.AppHost
```
...
```bash
dotnet run --project src/Host/Edvantix.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    ...
    Edvantix.DbMigrator/             One-shot migrate / seed runner
    Edvantix.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/Edvantix.Api/appsettings.json`.
...
```bash
dotnet test src/Edvantix.slnx       # integration tests require Docker
```

#!/usr/bin/env bash
#
# One-command deploy of the Edvantix Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/Edvantix.Api/Edvantix.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/Edvantix.DbMigrator/Edvantix.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \

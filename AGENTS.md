...
| `src/Host/Edvantix.Api` | Composition-root Web API host. |
| `src/Host/Edvantix.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/Edvantix.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/Edvantix.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/Edvantix.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/Edvantix.slnx                   # build backend
dotnet run --project src/Host/Edvantix.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/Edvantix.slnx                    # tests — integration tests REQUIRE Docker
...
dotnet run --project src/Host/Edvantix.DbMigrator -- list-pending
```
...
| A specific module's quirks | `modules/{module}.md` (identity, multitenancy, chat, files, webhooks, auditing, billing, curriculum, people, study-groups, scheduling, payments, tickets, notifications) |

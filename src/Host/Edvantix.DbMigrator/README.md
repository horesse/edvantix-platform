...
dotnet run --project src/Host/Edvantix.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/Edvantix.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/Edvantix.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/Edvantix.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/Edvantix.DbMigrator -- seed-demo
```
...
as `Edvantix.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `Edvantix.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/Edvantix.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: Edvantix.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/Edvantix.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/Edvantix.Api
  ```

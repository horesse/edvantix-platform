...
// Per-app prefix from the AppHost assembly name (Edvantix.AppHost -> fsh-starter); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var migrator = builder.AddProject<Projects.Edvantix_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "Edvantix.Migrations.PostgreSQL")
...
var demoSeeder = builder.AddProject<Projects.Edvantix_DbMigrator>($"{appPrefix}-demo-seeder")
    .WithReference(postgres)
    .WaitFor(postgres)
...
// API Service
var api = builder.AddProject<Projects.Edvantix_Api>($"{appPrefix}-api")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WaitFor(redis)
    ...
    .WithEnvironment("MailOptions__DisplayName", "Edvantix")

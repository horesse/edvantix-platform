...
- **Study-group channels** (`IntegrationEventHandlers/`, order 800 = after StudyGroups 610): `ChatChannel.CreateForStudyGroup` sets `SourceStudyGroupId`, `Slug = null` (group names aren't unique), `IsPrivate`. Sync handlers key off `SourceStudyGroupId` (partial index) — no StudyGroups→Chat runtime ref. `StudentUnenrolled` removal is guarded against a shared guardian-payer still active via another student (`IStudyGroupQueryService.GetActiveStudentIdsAsync` + re-resolve). `StudyGroupFinished` → `channel.Lock()`; `SendMessageCommandHandler` rejects a locked channel with 409 (`IsLocked` ≠ `IsDeleted`). Chat calls `AddIntegrationEventHandlers` and publishes `StudyGroupChannelLinkedIntegrationEvent` **directly on `IEventBus`** (same as the mention event — Chat has no outbox wiring). Student→chat identity: own `UserId` → primary-payer guardian → any guardian with an account → skip.
- **DM policy** (`Features/v1/Channels/DmPolicy/`): `IChatDmPolicy.CanStartDmAsync` gates `FindOrCreateDmCommandHandler` (per target id → 403). Staff (`Admin`/`SchoolAdmin`/`Manager` via `IUserService.GetUserRolesAsync`) bypass both ways; teacher↔teacher ok; student/guardian↔a teacher of their (ward's) groups ok both directions (`IStudyGroupQueryService` group-set intersection via `IPeopleScopeResolver`); student↔student behind `ChatDmSettings.AllowStudentToStudentDm` (one row per tenant, default off, `GET`/`PUT /chat/dm-settings`, `SchoolSettings` perms); else deny. Existing DMs are not retro-closed. Module now refs `Multitenancy.Contracts`.

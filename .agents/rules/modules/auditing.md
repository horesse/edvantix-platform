...
- **Manual reads are audited too** — non-CRUD sensitive reads call `IAuditClient.WriteActivityAsync(ActivityKind.Query, name, …)` from the query handler (interceptor is blind to reads). Current callers: `GetDebtorsReportQueryHandler` (Payments), `GetStudentNotesQueryHandler` (People). `responsePreview` carries only counts/totals — never the sensitive payload (note text). Those modules reference `Modules.Auditing.Contracts` for this.
...
- **Friendly labels** — the audit log records raw `ClrType.Name` / property names. `AuditLabelCatalog` (`Contracts/Catalog/`) maps them to RU labels; `GET /audits/entity-labels` serves `{ entities, fields }` as one source of truth for the UI. Not an allow-list — an unlabelled name falls back to itself (`EntityLabel`/`FieldLabel`). Add a key when you add a school-domain aggregate the UI opens a history for.
